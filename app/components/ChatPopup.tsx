...
import { Loader2, MessageCircle, Send, User, Bot, X } from "lucide-react";
...
export default function ChatPopup({ open, onClose }: ChatPopupProps) {
  ...
  return (
    <>
      {/* No backdrop - page remains accessible and scrollable */}
      {/* Chat Window */}
      <Card className="fixed bottom-6 right-6 w-96 h-[70vh] max-h-[600px] max-w-md flex flex-col z-50 shadow-2xl border-0 rounded-3xl animate-in slide-in-from-bottom-8 fade-in duration-200 sm:bottom-8 sm:right-8 p-0">
        ...
        <ScrollArea className="flex-1 p-0 sm:p-0 space-y-0 min-h-0 overflow-auto">
          {messages.length === 0 ? (
            ...
            messages.map((msg) => (
              ...
              >
                ...
                <div
                  className={cn(
                    "'max-w-full px-3 sm:px-4 py-2 rounded-2xl text-xs sm:text-sm prose prose-sm **:wrap-break-word [&_*]:overflow-wrap-break-word max-w-full m-2",
                    msg.role === "user"
          ...
          {isLoading && (
            <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 pl-3 sm:pl-11">
